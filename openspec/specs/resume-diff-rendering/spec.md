...
Defines how the resume canvas (`app/src/resume-engine`) computes and renders old-vs-new differences for a
pending agent proposal, directly in the rendered resume layout, across every template-engine node type (scalar
text, rich text, list leaf, list repeater, nested skill-groups) — and how this diff-mode rendering stays
isolated from the existing interactive/static/PDF rendering paths.
