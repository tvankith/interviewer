...
Defines the per-field/per-entry tri-state review workflow for candidate review of an agent-proposed resume
edit: independent accept/reject/pending decisions accumulated client-side per review unit, batched into a
single reconciled write, and reported back to the agent as per-unit outcomes — replacing whole-proposal
accept/reject.
